...
#
# Usage:
#   ./release.sh          # reads version from nera-self-exclusion-plugin.php (Version header)
...
# Pushes to `origin` (configure it once: git remote add origin git@github.com:Nera-Marketing/nera-self-exclusion-plugin.git).
#
...
#   3. Copies plugin to a clean temp dir; sets * Version + NERA_SE_VERSION; syncs readme Stable tag + plugin.json
...
PLUGIN_SLUG="nera-self-exclusion-plugin"
PLUGIN_MAIN="nera-self-exclusion.php"   # main plugin file basename (differs from folder/slug)
GITHUB_REPO="Nera-Marketing/nera-self-exclusion-plugin"
...
echo "▶ Setting * Version + NERA_SE_VERSION to ${VERSION} in release tree (for PUC)..."
...
if sed --version >/dev/null 2>&1; then
  ...
  sed -i "s/define( 'NERA_SE_VERSION', '[^']*' );/define( 'NERA_SE_VERSION', '${VERSION}' );/" "$PHP_MAIN"
else
  sed -i '' "s/^ \\* Version: .*/ * Version: ${VERSION}/" "$PHP_MAIN"
  sed -i '' "s/define( 'NERA_SE_VERSION', '[^']*' );/define( 'NERA_SE_VERSION', '${VERSION}' );/" "$PHP_MAIN"
fi
